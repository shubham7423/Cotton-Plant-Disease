...
checkpoint = ModelCheckpoint("Densenet201.h5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='min', period=1)
early = EarlyStopping(monitor='val_loss', min_delta=0, patience=30, verbose=1, mode='min')

#compile model
EPOCHS = 400
BS = 32
opt = Adam(learning_rate=0.001,  decay=1e-7)
...
model.save('model/Densenet201-final.h5')
